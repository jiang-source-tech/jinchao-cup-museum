...
设备连接层不再直接依赖 `XiaoxinRuntime`。新建一个小接口：
...
馆方既有二维码可以继续提供展品图文、音视频和延伸阅读，但它与小芯运行时保持独立。小芯不生成故事卡、数字纪念册、收藏二维码或参观后网页，也不把扫码访问作为现场对话的完成条件。

小芯的产品闭环必须在展品现场完成：确定当前展品、进行有依据问答、给出一个观察任务并推进参观路线。游客结束会话时，设备只显示简短现场回顾并清除上一小组的可见状态。
...
只有在博物馆主链路、后台、固件和部署均稳定后，才按调用关系删除 `core/xiaoxin` 中不再使用的旧业务代码和旧测试。不能在迁移前直接删除，以免破坏语音与设备基座中仍存在的耦合。
