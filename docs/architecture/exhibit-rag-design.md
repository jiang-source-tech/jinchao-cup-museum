...
- 状态：`implemented`（服务端文本链路）；真机语音闭环仍待验收
...
本文基于已经确认的硬件和现场约束定义系统：设备不能通过固定点位、视觉识别、定位信号或触摸选择可靠地知道游客正在观看哪件展品；游客仍按馆内既有标识和个人兴趣自然参观；系统只在游客主动说出展品并提问时提供可信、适合语音播放的讲解。

当前服务端已经实现从用户问题解析展品、临时会话继承、SQLite 与 Qdrant 混合检索、异构资料摄取、原文片段检索、声明级引用校验和四档回答深度。仓库候选内容为 101 件展品、187 条事实和 108 个来源，其中 3 件重点展品已建立章节级公开资料证据包。尚未完成的是当前新增内容的生产发布、完整黄金评测和真机 ASR/RAG/TTS 闭环验收。

## 1. 结论

产品不定位为替游客安排参观顺序的路线导览系统，也不要求设备自动感知展品。产品定位为：

> 面向儿童、亲子家庭和普通游客的展品级可信语音讲解伙伴。游客在问题中说出展品，系统从已审核且有来源的内容中检索依据，以与问题和资料量匹配的方式回答，并在临时会话中支持围绕同一展品继续追问。

技术定位为：

> 以展品实体解析为入口、以已发布事实和可定位原文片段为依据、以声明级引用守卫约束回答的领域 RAG 系统。
...
- 具体事实必须来自馆方认可资料、出版物、研究材料、权威公开页面或其他可追溯来源；当前 Demo 使用公开资料时必须明确其来源等级。
- 模型常识不是展品事实来源。
...
8. 根据本轮问题输出 `brief`、`standard`、`guided` 或 `detailed` 档位的回答，长度必须与资料量匹配。
...
- 展品别名维护和歧义检查。
- 未命中问题聚合和内容缺口回收。
- 稳定的四档讲解策略，支持游客通过自然语言临时要求更简单或更详细。
- 跨展品比较，但必须明确识别两个展品并分别建立依据快照。
- 扩展重点展品原文证据包、黄金问题集和证据引用评测。
...
> 关于战国水晶杯，当前资料还没有确认它具体属于谁，我不能替它补一个名字。
...
- `brief` 通常 1 至 2 句，`standard` 通常不超过 4 句；
- `guided` 在资料充足时通常 5 至 9 句，`detailed` 在资料充足时通常 8 至 12 句；
- 资料不足时宁可缩短或拒答，不能为了达到句数重复、扩写常识或虚构故事；
- 优先直接回答问题；
- 使用儿童能够理解的词语；
- 必要时解释一个专业词；
- 最多提供一个可选观察提示；
...
evidence_segments
  ...
  -> Qdrant 片段向量
  -> RRF 融合
  -> claim_support / conflict 约束
```

两条链路都要求 lexical 与 dense 候选回到 SQLite 复核活动状态、内容版本和来源边界。Qdrant 只负责候选召回，不能绕过发布状态、来源撤回、声明支持关系和回答守卫；dense 分支失败时允许降级到 lexical，并必须记录降级原因。

### 11.3 后端启用边界

当前仓库候选内容为 101 件展品，其中 3 件重点展品已经建立原文证据包，另外 15 件只有事实级多维内容，83 件只有目录级登记。`config.example.yaml` 默认仍使用 `legacy_facts`，`evidence_segments` 需要显式启用，避免把未完成证据摄取的展品误写成全量原文 RAG。

只有对应展品完成来源 manifest、片段摄取、索引构建、`claim_support` 和回归评测后，才能把它计入原文证据 RAG 覆盖率。扩大 Qdrant 点数不等于扩大可信知识覆盖。
...
- 实际检索候选和最终引用的 `evidence_ids`；
- 每条回答声明与支持片段的对应关系；
- 来源版本、章节或页码定位和内容哈希；
- 证据状态是 `supported`、`partial`、`conflicting` 还是 `unsupported`。
...
> 关于战国水晶杯，当前资料还没有确认它具体属于谁，我不能替它补一个名字。你可以问我它的年代、材料或制作难点。
...
- 从用户问题解析展品、歧义澄清和临时会话继承；
- 事实级 SQLite FTS、Qdrant 和 RRF Hybrid 检索；
- PDF、Markdown/TXT、JSON、HTML 和 OCR sidecar 摄取；
- 原文片段、来源不可变版本和来源撤回；
- 片段级 SQLite FTS、Qdrant 和 RRF Hybrid 检索；
- `claim_support`、声明级证据引用和冲突约束；
- `brief`、`standard`、`guided`、`detailed` 四档回答；
- 有依据回答和知识兜底；
- 依据快照和交互审计；
- `museum_state`；
- 服务端与固件已有 ASR、TTS、播放和状态传输代码，但当前 RAG 版本尚未完成真机闭环验收。

### 尚未完成

- 将原文证据包从当前 3 件重点展品扩展到下一批代表性展品；
- 建立覆盖检索命中、引用正确性、拒答和回答自然度的固定黄金评测；
- 让来源撤回自动触发证据向量索引重建，目前仍依靠实时过滤和显式重建；
- 将当前新增内容经过正式生命周期发布并完成生产验收；
- 同时核对服务端与固件后完成真机 ASR/RAG/TTS、打断、重连和屏幕状态验收。
...
拒绝原因：项目已经使用 Qdrant，但向量检索仍只是受展品、版本、来源和状态约束的候选召回器。不能为了宣称“用了向量数据库”而跳过结构化过滤、词法召回、引用校验和评测门禁。

## 20. 文档一致性状态

PRD、根 README、文档中心、内容目录和本设计已经按当前四档回答、101 件候选内容、Hybrid 检索和原文证据链路更新。历史阶段报告继续保留当时的生产数字和验收结论，不应改写为当前仓库实时状态。

后续文档更新仍必须区分：仓库候选内容、已经部署的生产版本、历史验收快照和真机实际观察。服务端文本链路通过不能写成麦克风、ASR、TTS、扬声器或屏幕已经验收。
