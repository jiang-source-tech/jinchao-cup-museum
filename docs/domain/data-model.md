...
FTS5 派生索引保存事实检索所需字段，不作为事实真源。

| 字段 | 含义 |
| --- | --- |
| `fact_id` | 对应事实 ID，不参与全文分词 |
| `exhibit_id` | 所属展品，用于 SQL 层隔离 |
| `revision_id` | 所属内容版本，用于限定当前发布版本 |
| `exhibit_name` | 展品规范名称 |
| `aliases` | 展品别名文本 |
| `fact_type` | 事实类型 |
| `statement` | 事实陈述 |
| `keywords` | 内容人员审核的检索关键词 |

旧索引缺少 `exhibit_id` 或 `revision_id` 时，初始化过程会删除旧 FTS 表，并从 `exhibit_fact`、`content_revision` 和 `exhibit` 重新构建全部索引行。历史版本索引可以保留，但查询必须同时限定当前展品和当前发布 revision，不能依赖应用层事后过滤。
...
9. 内容发布和撤回必须留下审计记录。
10. FTS 候选必须同时属于本轮展品和当前发布版本。

## 第一版检索

第一版按以下顺序检索：

1. 得到唯一 `current_exhibit_id` 和当前发布 `revision_id`；
2. 在 SQL/FTS5 查询中同时限定展品、版本、有来源事实和允许的事实类型；
