...
http://localhost:8002/xiaoxin/doc.html
```
...
```
manager-api:
  url:  http://127.0.0.1:8002/xiaoxin
  secret: 你的server.secret值
```
1、把你刚才从`智控台`复制过来的`server.secret`的`参数值`复制到`.config.yaml`文件里的`secret`里。

2、因为你是docker部署，把`url`改成下面的`http://xiaozhi-esp32-server-web:8002/xiaoxin`

3、因为你是docker部署，把`url`改成下面的`http://xiaozhi-esp32-server-web:8002/xiaoxin`

4、因为你是docker部署，把`url`改成下面的`http://xiaozhi-esp32-server-web:8002/xiaoxin`

类似这样的效果
```
manager-api:
  url: http://xiaozhi-esp32-server-web:8002/xiaoxin
...
```
25-02-23 12:01:09[core.websocket_server] - INFO - Websocket地址是      ws://xxx.xx.xx.xx:8000/xiaoxin/v1/
...
OTA接口：
```
http://你宿主机局域网的ip:8002/xiaoxin/ota/
```

Websocket接口：
```
ws://你宿主机的ip:8000/xiaoxin/v1/
```
...
```
25-02-23 12:01:09[core.websocket_server] - INFO - Server is running at ws://xxx.xx.xx.xx:8000/xiaoxin/v1/
25-02-23 12:01:09[core.websocket_server] - INFO - =======上面的地址是websocket协议地址，请勿用浏览器访问=======
25-02-23 12:01:09[core.websocket_server] - INFO - 如想测试websocket请启动digital-human模块，打开浏览器交互测试
25-02-23 12:01:09[core.websocket_server] - INFO - =======================================================
```
...
OTA接口：
```
http://你电脑局域网的ip:8002/xiaoxin/ota/
```

Websocket接口：
```
ws://你电脑局域网的ip:8000/xiaoxin/v1/
```
