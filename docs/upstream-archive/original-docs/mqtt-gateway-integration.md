...
```
ws://127.0.0.1:8000/xiaoxin/v1/?from=mqtt_gateway
```

2、如果你是docker部署，你的`mqtt-websocket`地址是
```
ws://你宿主机局域网IP:8000/xiaoxin/v1/?from=mqtt_gateway
```
...
上面的配置完成后，你可以使用curl命令，验证你的ota地址是否会下发mqtt配置，把下面的`http://localhost:8002/xiaoxin/ota/`改成你的ota地址
```
curl 'http://localhost:8002/xiaoxin/ota/' \
