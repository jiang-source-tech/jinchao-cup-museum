...
- 本轮性质：本地清理与提交已完成；不执行 push、生产部署或 OTA 发布
- 总体置信水平：
  - 本地源码、数据库、构建产物与 Git 状态结论：高
  - “八点高等数学”具体由哪一台线上实例、哪一份线上数据库触发：中
...
- 生产服务器目录、容器、数据挂载、健康入口和设备实际固件 SHA 尚未确认，因此尚未执行生产止血、部署或 OTA 发布。

本地阶段状态：阶段 1、2、4 和 7 已完成主要源码清理、验证和本地提交；配置模板、缓存失效、旧通知入口和版本迁移已完成。阶段 0、3 和 6 涉及生产取证、数据切换、发布和部署，仍未执行。
...
在读取生产环境前，不能断言具体是哪一个容器或数据库记录。需要核对：

1. 真实 Compose 目录和容器名称。
2. 容器镜像 ID、启动时间、命令、挂载目录。
3. 生产服务端 Git HEAD 和工作区状态。
4. 生产 data/.config.yaml 中旧调度开关的实际值。
5. 生产 xiaoxin_control.db 的 student_courses、课程提醒设置和设备绑定记录。
6. 提醒发生时间前后 10 分钟的服务端日志。
7. 设备实际运行分区的应用 SHA，而不仅是版本字符串 0.1.4。

当前最可能的解释是：旧服务端实例读取生产持久化数据库中的课程记录并通过 TTS 下发。置信水平为中，完成生产取证后才能提升为高。
