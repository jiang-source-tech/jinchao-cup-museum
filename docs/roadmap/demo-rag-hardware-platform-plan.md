...
已完成 3 件首轮重点展品的服务端纵向切片：

- 战国水晶杯、玉钺组合和玉三叉形器已建立 10 个公开来源、26 条原子事实和章节级 `claim_support`；
...
- 回答已支持 `brief`、`standard`、`guided`、`detailed` 四档，导览和详细档能够按证据组织更完整的现场讲解。

当前尚未达到完整 Demo 完成定义：

- 只完成 3 件首轮验证集，还没有扩展到 15–20 件代表性展品；
- 已有聚焦回归，但还没有独立的完整黄金问题集、证据评测命令和指标报告；
- `evidence_segments` 仍是显式选择的后端，默认继续使用 `legacy_facts`；
- 来源撤回尚未自动触发 Qdrant 重建；当前先实时过滤撤回来源，再由下一次显式重建清理向量副本；
- 本轮新增内容和能力尚未推送、部署或执行生产发布生命周期；
