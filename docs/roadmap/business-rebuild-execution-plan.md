...
- 当前状态：阶段 A、B 已完成；服务端展品解析、事实级与片段级 Hybrid RAG、声明级引用和四档回答已实现；3 件重点展品证据包已完成。阶段 D 至 F 的真机动作、协议和播放仍待按当前服务端与固件提交复验，阶段 G 的内容运营后台不在当前 Demo 范围内。
...
6. 实现 `brief`、`standard`、`guided`、`detailed` 四档儿童友好回答。
7. 实现资料不足兜底。
...
主链路稳定后再建设展品事实与来源、审核发布、设备点位、路线配置，以及问题、依据、兜底和延迟记录。当前仓库已有 101 件候选展品，但应先把 3 件重点展品和下一批常规多维展品的原文证据做深；无真实馆方授权时必须标记为演示数据。
...
当前服务端下一步是扩展重点展品证据包、黄金问题集和引用评测；硬件侧仍需把 `museum_action`、`museum_action_result`、真机 ASR/RAG/TTS 和状态展示按当前双仓库提交重新验收。阶段 G 的馆方后台和内容运营继续保持非 Demo 核心范围。
