...
`visitor_session_id` 可以为空，表示需要根据设备和明确规则创建或恢复会话。不得把 `transport_session_id` 当作游客身份。
...
服务端建议增加 1 个 `TurnOutcome` 到 `museum_state` 与 TTS 队列的接入测试。固件先做主机侧合同解析冒烟检查和真机手工验收。
...
实现普通、亲子、深度三种表达策略、短期追问上下文、展品歧义确认，以及长度、问题预算和资料外具体事实守卫。守卫失败最多修复一次，再失败直接兜底。

验收门槛：同一问题三种模式表达不同但事实 ID 不冲突，连续追问不错误切换展品。
...
2026 年 8 月 9 日已完成阶段 A：

1. 新建通用运行时合同；
2. 新建运行时工厂；
3. 用 legacy adapter 保持现有行为；
4. 把 `ConnectionHandler.chat()` 改为调用统一入口；
5. 增加两个聚焦测试，分别保护 legacy 适配和新运行时回复进入既有 TTS 队列；
6. 运行聚焦测试和最小语法检查。

2026 年 8 月 9 日已完成阶段 B：

1. 新建 `core/museum` 深模块，包含 SQLite 存储、事实检索、回答编排和运行时；
2. 导入战国水晶杯演示展品、六条事实、两个来源和发布版本；
3. 通过设备点位确定当前展品，并建立 30 分钟临时游客会话；
4. 只允许发布版本且绑定来源的事实进入依据快照；
5. 对资料外问题返回明确兜底，博物馆模式始终 `handled=True`；
6. 保存问题、事实 ID、来源 ID、内容版本、回答和耗时；
7. 增加已发布事实、资料外兜底和缺少当前展品三条聚焦测试。
8. 比赛运行配置必须显式设置 `type: museum`；代码层继续以 `legacy` 作为过渡期缺省值，避免未完成真机配置时误切换基座行为。

当前下一步是阶段 C：把已有 `display_state` 转成固件 `museum_state` 消息，并在目标板完成真实 ASR、TTS 和屏幕闭环。阶段 C 开始前需要确认真机 `device_id` 和本地启用配置。
