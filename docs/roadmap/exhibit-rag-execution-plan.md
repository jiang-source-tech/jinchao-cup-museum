...
- 2026 年 8 月 11 日进度：服务端文本自由对话验收入口已实现；真实 LLM 和真机链路仍待配置与现场验证
...
当前未配置 `selected_module.LLM`，因此默认以 `deterministic-only` 模式运行。使用
`--require-llm` 会在没有真实 LLM 配置时以退出码 2 明确失败，不能把规则模式冒充为模型验收。
