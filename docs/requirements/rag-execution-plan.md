...
| 当前结论 | 事实级与原文片段级 RAG 均已实现；默认继续使用 `legacy_facts`，重点展品按证据包和评测结果逐步切换 `evidence_segments` |
| 下一项实施需求 | 扩展下一批重点展品的原文证据包、黄金问题集和真机前置检查 |

这不是“未来愿景”或技术名词清单，而是一份可以逐项执行、验收和归档的任务单。每个任务都写明输入、修改边界、验收条件和完成证据。完成状态仍以 `docs/requirements/requirements.yaml` 为准。

当前执行状态：`RAG-NEXT-01` 至 `RAG-NEXT-06` 已完成；3 件重点展品的原文证据摄取、片段检索和声明级引用已实现，但完整证据评测、下一批内容扩展和真机前置检查仍待完成。
...
4. 对每件重点展品补齐来源片段、`claim_support` 和引用覆盖评测；
5. 核心真机闭环稳定后，再扩展下一批常规多维展品，而不是批量制造同质目录资料。
...
- 不直接对全馆长文档开放检索；
- 不用草稿、撤回版本或无来源事实回答；
- 不因“项目叫 RAG”就绕过内容审核、来源绑定和引用校验；当前 Qdrant 只作为受约束的候选召回器。
...
| 8 | RAG-NEXT-08 原文证据 RAG 覆盖扩展 | REQ-018 | 05/06 | 重点展品具备片段检索、声明引用和冲突边界 |
| 9 | RAG-NEXT-09 扩展下一批重点展品 | REQ-017 | 03/05 | 按证据模板和评测门禁扩充，不阻塞 P0 闭环 |
...
**实现边界**：保留 `MuseumStore` 的 SQLite + FTS5 作为结构化事实源，并在受展品、版本和来源约束的范围内使用 Qdrant Hybrid；向量命中不能直接成为回答依据。
...
- `tests/fixtures/museum_conversation_eval.json` 保存 35 个用例、45 轮固定输入，覆盖 5 件杭州公开来源展品、12 个审核别名、5 个 ASR 误识别别名、多轮追问、显式切换、歧义、未收录、资料不足和诱导编造；
...
### RAG-NEXT-08：原文证据 RAG 覆盖扩展

**关联**：`REQ-018 / AC-018-2`。

当前已在 3 件重点展品上建立片段级证据 RAG。下一步不是重新讨论是否使用 Embedding，而是扩大高质量证据覆盖并验证引用正确性。当前配置默认 `legacy_facts`，只有完成来源 manifest、片段摄取、索引构建、`claim_support` 和回归评测的展品才切换到 `evidence_segments`。
...
4. 必须用同一离线评测集比较事实级与片段级检索的召回率、引用覆盖率、错误绑定率、延迟和可解释性；
5. 任何新增展品都必须同时提交正向问题、拒答问题、干扰问题和来源反查结果；
6. 没有量化收益时，不扩大证据索引范围或全馆开放检索。

### RAG-NEXT-09：扩展下一批重点展品
...
**验收**：下一批展品均可导入、审核、发布和回滚，并完成对应的证据覆盖、引用校验、拒答和干扰评测；扩容不降低单展品事实边界、审计完整性和未命中分类质量。

## 5. 推荐的实际执行顺序

下一步不要先批量录入目录，也不要把向量命中当作最终答案依据。按下面顺序推进：

1. `RAG-NEXT-01` 已完成：建立 3 件展品的最小内容 fixture 和内容契约；
2. `RAG-NEXT-02` 已完成：导入 CLI、完整校验和草稿事务；
3. `RAG-NEXT-03` 已完成：发布、撤回、回滚、生命周期事件和历史版本复核；
4. `RAG-NEXT-04` 已完成：事实级检索在 SQL 层绑定展品和发布版本，且保持跨展品隔离；
...
7. 执行 3 件重点展品的证据引用评测，并扩展下一批常规多维展品；
8. 同步执行 `RAG-NEXT-07` 的服务端、协议和固件前置检查，硬件接入后再完成真实链路证据。
