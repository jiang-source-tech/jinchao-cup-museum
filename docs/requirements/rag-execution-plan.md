...
| 下一项实施需求 | `REQ-013`：内容导入、发布校验与版本回滚 |
...
当前真正缺的不是“再加一个 RAG 框架”，而是：

1. 内容人员不改 Python 代码就能导入和发布多件展品；
2. 检索在多展品、自然问法和资料不足场景下有固定评测集；
3. 真实 LLM 输出持续被事实 ID、来源和回答守卫约束；
4. 未命中结果能区分展品未识别、事实缺口、范围外和 ASR 错误；
5. 有量化证据后再决定是否需要向量召回。
...
1. 先实现 `RAG-NEXT-01`：建立 3 至 5 件展品的最小内容 fixture 和内容契约；
2. 紧接着实现 `RAG-NEXT-02`：导入 CLI，先完成 `validate` 和草稿事务；
3. 实现 `RAG-NEXT-03`：发布、撤回、回滚和历史版本验证；
4. 用这些内容跑 `RAG-NEXT-04`：确认 FTS5 + 事实类型约束没有跨展品泄漏；
