...
class FirmwareReleaseCatalog:
    ...
    def from_config(
        ...
        *,
        ...
        release_config = control.get("ota_release", {}) or {}
        root = Path(project_dir) if project_dir is not None else Path.cwd()

        database_path = cls._resolve_path(
            root,
            release_config.get("db") or "data/xiaoxin_firmware_releases.db",
        )
        artifact_dir = cls._resolve_path(
            root,
            release_config.get("artifact_dir") or "data/xiaoxin_firmware",
        )
    ...
    def _normalize_public_ota_url(
        ...
        *,
        ...
        if (
            ...
            or parsed.query
            or parsed.fragment
            or not parsed.path.rstrip("/").endswith("/xiaoxin/ota")
        ):
            raise FirmwareReleaseError(
                "public_ota_url must be an HTTPS /xiaoxin/ota/ URL; "
