"""Terminal shell for the throwaway companion narrative prototype."""
...
def _render_timeline(run: ScenarioRun) -> None:
    ...
    for item in run.timeline:
        ...
        print(
            "  academic="
            f"{item['academic_stage']}/{item['academic_status']} "
            f"age={item['xiaoxin_age']} posture={item['posture']}"
        )
        print(
            "  records="
            f"boundaries:{len(item['boundaries'])} "
            f"chapters:{len(item['chapters'])} "
            f"moments:{len(item['moments'])} "
            f"visible_history:{len(item['miniprogram_history'])}"
        )
        if item["moments"]:
            latest = item["moments"][-1]
            projection = latest["projection"]
            print(
                "  latest="
                f"{latest['primary_kind']}/{latest['mode']}/{latest['status']} "
                f"voice:{projection['voice']['max_sentences']} "
                f"hardware:{projection['hardware']['semantic']}"
            )
...
def _render(index: int, message: str = "") -> None:
    ...
    print(f"{BOLD}COMPANION NARRATIVE V1 - THROWAWAY{RESET}")
    print(
