"""Terminal shell for the throwaway relationship timeline prototype."""
...
def _render_timeline(run: ScenarioRun) -> None:
    ...
    for item in run.timeline:
        ...
        print(f"- {item['label']} @ {item['now']}")
        print(
            "  stage="
            f"{item['stage']} legacy={item['legacy_stage']} "
            f"posture={item['posture']}"
        )
        print(
            "  span="
            f"{metrics['elapsed_days']}d active={metrics['active_days']}d/"
            f"{metrics['active_weeks']}w/{metrics['active_months']}m "
            f"knowledge={metrics['knowledge_items']} "
            f"helpful_days={metrics['helpful_days']} "
            f"attunement_days={metrics['attunement_days']}"
        )
        print(
            "  projection="
            f"memory:{projection['memory_reference_budget']} "
            f"initiative:{projection['initiative_level']} "
            f"gain:{projection['implicit_adjustment_gain']}"
        )
...
def _render(index: int, message: str = "") -> None:
    ...
    print(f"{BOLD}RELATIONSHIP TIMELINE V1 - THROWAWAY{RESET}")
    print(
