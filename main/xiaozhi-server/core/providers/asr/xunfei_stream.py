...
class ASRProvider(ASRProviderBase):
    ...
    def _ensure_decoder(self):
        if self.decoder is None:
            self.decoder = _get_opuslib_next().Decoder(16000, 1)
        return self.decoder
    ...
    async def receive_audio(self, conn: "ConnectionHandler", audio, audio_have_voice):
        ...
        if self.asr_ws and self.is_processing and self.server_ready:
            try:
                pcm_frame = self._ensure_decoder().decode(audio, 960)
    ...
    async def _start_recognition(self, conn: "ConnectionHandler"):
        ...
        try:
            ...
            if conn.asr_audio and len(conn.asr_audio) > 0:
                ...
                pcm_frame = (
                    self._ensure_decoder().decode(first_audio, 960) if first_audio else b""
                )
                ...
                for cached_audio in conn.asr_audio[-10:]:
                    try:
                        pcm_frame = self._ensure_decoder().decode(cached_audio, 960)
                        await self._send_audio_frame(pcm_frame, STATUS_CONTINUE_FRAME)
                    except Exception as e:
