...
UPSTREAM_DEFAULT_SIMILARITY_THRESHOLD = 0.2
...
class VoiceprintProvider:
    ...
    def __init__(
        self,
        config: dict,
        speaker_resolver: Callable[[str | None], list[tuple[str, str]]] | None = None,
    ):
        self.original_url = config.get("url", "")
        self.speakers = config.get("speakers", [])
        self.speaker_map = self._parse_speakers()
        # Keep the adapter default aligned with the pinned voiceprint-api model.
        # A stricter deployment-specific threshold can still be configured.
        self.similarity_threshold = float(
            config.get(
                "similarity_threshold", UPSTREAM_DEFAULT_SIMILARITY_THRESHOLD
            )
        )
        ...
        if not self.original_url:
            ...
            if not self.api_key:
                ...
                # 检查是否有有效的说话人配置
                if not self.speaker_ids and self.speaker_resolver is None:
    ...
    def _dynamic_speakers(
        self, device_id: str | None
    ) -> tuple[list[str], Dict[str, Dict[str, str]]]:
        if self.speaker_resolver is None:
            return [], {}
        try:
            resolved = self.speaker_resolver(device_id)
        except Exception as exc:
            logger.bind(tag=TAG).warning(f"动态声纹候选加载失败: {exc}")
            return [], {}
        speaker_ids = []
        speaker_map: Dict[str, Dict[str, str]] = {}
        for speaker_id, display_name in resolved:
            if not speaker_id or speaker_id in speaker_map:
                continue
            speaker_ids.append(speaker_id)
            speaker_map[speaker_id] = {"name": display_name or speaker_id}
        return speaker_ids, speaker_map
    ...
    async def identify_speaker(
        ...
        try:
            ...
            dynamic_ids, dynamic_map = self._dynamic_speakers(device_id)
            # Once a resolver is installed, its device-scoped result is the
            # only trusted candidate set. Never fall back to the legacy
            # global list when a device has no enrolled speakers or the
            # resolver failed; doing so can turn a test/static identity into
            # a confirmed private-memory subject.
            if self.speaker_resolver is not None:
                speaker_ids = dynamic_ids
                speaker_map = dynamic_map
            else:
                ...
                async with session.post(self.api_url, headers=headers, data=data) as response:
                    ...
                    if response.status == 200:
                        ...
                        if speaker_id and speaker_id in speaker_map:
                            if dynamic_ids:
                                # Keep the provider ID stable for identity resolution.
                                result_name = f"voiceprint:{speaker_id}"
                            else:
                                result_name = speaker_map[speaker_id]["name"]
