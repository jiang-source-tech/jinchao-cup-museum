...
本目录保存可直接通过内容导入 CLI 校验和导入的公开资料摘要与官网目录级登记。当前没有真实馆方对接，这些内容不代表馆方授权、审核或正式发布。所有 revision 均保持 `draft`，必须经过项目既有 `review` 和 `publish` 发布门后才能进入游客回答。

2026 年 8 月 15 日批量审计结果为：8 个内容包、4 家馆、101 件展品、187 条事实和 108 个来源。完整分级和逐件可提问范围见 [`../../EXHIBIT_CATALOG.md`](../../EXHIBIT_CATALOG.md)。
...
| `china-national-silk-museum-stage3-catalog.json` | 中国丝绸博物馆 | 83 件官网目录级展品，每件仅有 1 条登记事实 |

其中战国水晶杯、玉钺组合和玉三叉形器已经补充章节级公开资料证据包；其余 15 件多维展品尚未达到相同的原文证据深度，83 件目录级展品不得宣传为可深度讲解。
