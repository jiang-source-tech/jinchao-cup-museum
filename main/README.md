...
3.  [核心组件深度剖析](#3-核心组件深度剖析)
    ...
    *   [3.4. `digital-human` (数字人测试模块 - Python+Web实现)](#34-digital-human-数字人测试模块---pythonweb实现)
...
5.  **`digital-human` (数字人测试模块 - Python+Web实现):**
    ...
    *   **FFmpeg (外部依赖):** 在 `app.py` 启动时会进行检查 (`check_ffmpeg_installed()`)。FFmpeg通常用于音频处理和格式转换，例如，确保音频数据符合特定AI服务的要求或进行内部处理。
    ...
    2.  **WebSocket通信与连接处理 (`app.py`, `core/websocket_server.py`, `core/connection.py`):**
        *   **服务器启动与入口 (`app.py`):**
            ...
            *   使用 `asyncio.create_task()` 并发启动 `WebSocketServer` (监听如 `ws://0.0.0.0:8000/xiaoxin/v1/`) 和 `SimpleHttpServer` (监听如 `http://0.0.0.0:8003/xiaoxin/ota/`)。
    ...
    4.  **插件化功能扩展系统 (`plugins_func/`):**
        ...
        *   **实现机制:**
            *   各个具体功能以独立的Python脚本形式存在于 `plugins_func/functions/` 目录中（例如 `play_music.py`，以及用于Home Assistant集成的 `hass_set_state.py`）。
    ...
    6.  **辅助HTTP服务 (`core/http_server.py`):**
        *   与WebSocket服务并行运行一个简单的HTTP服务器，用于处理特定的HTTP请求。最主要的功能是为ESP32设备提供OTA (Over-The-Air) 固件更新的下载服务 (通过 `/xiaoxin/ota/` 端点)。此外，也可能承载其他如 `/mcp/vision/explain` (视觉分析) 等工具性HTTP接口。
...
*   **核心技术栈:**
    ...
    *   **Knife4j:** 一个集成了Swagger并增强了UI的API文档生成工具，专为Java MVC框架（尤其是Spring Boot）设计。它能生成美观且易于交互的API文档界面（通常通过 `/xiaoxin/doc.html` 访问）。
...
*   **关键实现细节:**
    ...
    6.  **API文档:**
        *   完整的API接口文档可通过以下地址访问: https://2662r3426b.vicp.fun/xiaoxin/doc.html
...
### 3.4. `digital-human` (数字人测试模块 - Python+Web实现)
...
*   **连接建立与握手:**
    *   ESP32设备作为客户端，主动向`xiaozhi-server`的指定端点（例如 `ws://<服务器IP>:<WebSocket端口>/xiaoxin/v1/`）发起WebSocket连接请求。
...
*   **OTA固件更新流程 (概念性描述):**
    ...
    *   当管理员触发对特定设备的OTA更新时：
        ...
        *   ESP32设备收到指令后，通过HTTP GET请求从该URL下载固件。此URL可能指向`xiaozhi-server`自身运行的`SimpleHttpServer`所服务的路径（如`/xiaoxin/ota/`），或者在某些架构中，也可能直接指向`manager-api`或专用的文件服务器。
