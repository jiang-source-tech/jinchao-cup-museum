...
if check_installed; then
    if whiptail --title "已安装检测" --yesno "检测到小智服务端已安装，是否进行升级？" 10 60; then
        ...
        docker compose -f /opt/xiaozhi-server/docker-compose.yml down
        ...
        # 下载最新版配置文件
        check_and_download "/opt/xiaozhi-server/docker-compose.yml" "https://ghfast.top/https://raw.githubusercontent.com/xinnan-tech/xiaozhi-esp32-server/refs/heads/main/main/xiaozhi-server/docker-compose.yml"
        
        # 启动Docker服务
        echo "开始启动最新版本服务..."
        # 升级完成后标记，跳过后续下载步骤
        UPGRADE_COMPLETED=1
        docker compose -f /opt/xiaozhi-server/docker-compose.yml up -d
    else
...
echo "------------------------------------------------------------"
echo "正在检查服务启动状态..."
if ! docker inspect -f '{{.State.Running}}' xiaozhi-esp32-server 2>/dev/null | grep -q true; then
...
WebSocket 地址: ws://$LOCAL_IP:8000/xiaoxin/v1/\n\
\n安装完毕！感谢您的使用！\n按Enter键退出..." 16 70
